...
You can run this project in an isolated Docker container for easier deployment.
...
```bash
docker pull virajsh/solar-radiation-app:latest
docker run -d -p 5000:5000 virajsh/solar-radiation-app:latest
```
...
### Build the Docker Image (Manual Option)

If you prefer to build the image yourself:

```bash
docker build --no-cache -t solar-radiation-app .
docker run -d -p 5000:5000 solar-radiation-app
```
...
* The image uses **Gunicorn** as the production server.
* `.dockerignore` ensures unnecessary files (like virtual environments, cache, and large datasets) aren’t copied into the image.
* You can modify the exposed port in the `Dockerfile` or the run command if needed:

  ```bash
  docker run -d -p 8080:5000 solar-radiation-app
  ```
